"""Lazy policy: ifdef at index by default."""
...
def test_lazy_default_on(monkeypatch):
    ...
    assert lazy_index_ifdef() is True


def test_lazy_index_applies_ifdef_by_default(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("HIERWALK_LAZY", raising=False)
    monkeypatch.delenv("HIERWALK_LAZY_IFDEF", raising=False)
    rtl = tmp_path / "top.v"
    rtl.write_text(
...
def test_lazy_index_ifdef_opt_out(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("HIERWALK_LAZY", "1")
    monkeypatch.setenv("HIERWALK_LAZY_IFDEF", "0")
    rtl = tmp_path / "top.v"
    rtl.write_text(
        "`define ON 1\n"
        "`ifdef ON\n"
